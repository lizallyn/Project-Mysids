...
summary(glmm.m)
# month only model not currently running, same error as null model
# glmm.1 <- glmmTMB(data = data, whalecount ~ StudyMon + (1|Site), family = truncated_nbinom1, ziformula = ~.)
# summary(glmm.1)
...
AIC.table <- kable(AICtab(glmm.mm, glmm.m), caption = "Table 1: delta AIC values for the three candidate models. glmm.mm included both mysid counts and study month as predictors, glmm.m included only mysid counts as a predictor, glmm.1 is the model with only study month as a predictor, and glmm.0 is the null intercept model.")
...
We checked for goodness of fit with the mysid model using the DHARMa package (Hartig, 2022) as well as visual checks on the plotted model predictions. The mysid model contains quantile deviations, and those are not addressed in this analysis. 
...
scatterplot <- ggplot() + 
  geom_point(data = data, aes(x = MysidCount, y = whalecount, color = Site), size = 3) + 
  ...
  labs(x = "Mysid Count per Sample", y = "Number of Whales near Site during Sample", 
       caption = "Figure 1: Scatterplot of observed whales by mysid counts, with predictions from the mysid model overlaid.") +
  theme.Speciesym
scatterplot
```

The model with mysid counts as a predictor did not perform better than the null intercept model, though the delta AIC value was only xxxx.
