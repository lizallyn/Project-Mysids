...
# hurdle
m.0 <- glmer(whalepresence ~ scale(MysidCount) + StudyMon + (1|Site), data = data, family = binomial)
summary(m.0)

# non-zero count data
m.non0 <- glmer.nb(whalecount ~ scale(MysidCount) + StudyMon + (1|Site), data = nonzero)
summary(m.non0)
```
